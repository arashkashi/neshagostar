	...
	foreach ($_POST['rows'] as $num=>$row) {		
		...
		if (empty($validateResult)) {
			...
			elseif ($row['action']=='update') {
				$result=$userSystem->update($columnsValues,$_GET['id']);
				$error = cmfcMySql::error();
				$msg =wsfGetValue('updateMsg');
			} 
			elseif ($row['action']=='insert') {
				$result=$userSystem->insert($columnsValues);
				$error = cmfcMySql::error();
...
if (in_array($_REQUEST['action'],array('new','edit')) and $saved!=true) {
	...
	$infa->showInputRow(wsfGetValue('username'),"rows[$num][columns][username]", $_POST['rows'][$num]['columns']['username'],'',40,'ltr');
	$infa->showInputPasswordRow(wsfGetValue('password'),"rows[$num][columns][password]", $_POST['rows'][$num]['columns']['password'],'',40,'ltr');
	$infa->showInputPasswordRow(wsfGetValue('repeatPassword'),"rows[$num][columns][passwordConfirmation]", $_POST['rows'][$num]['columns']['passwordConfirmation'],'',40,'ltr');
	$infa->showInputRow(wsfGetValue('email'),"rows[$num][columns][email]", $_POST['rows'][$num]['columns']['email'],'',40,'ltr');
	...
	$infa->showInputRow(wsfGetValue('firstName'),"rows[$num][columns][firstName]", $_POST['rows'][$num]['columns']['firstName'],'');
	$infa->showInputRow(wsfGetValue('lastName'),"rows[$num][columns][lastName]", $_POST['rows'][$num]['columns']['lastName'],'');
