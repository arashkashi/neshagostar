...
if (in_array($_REQUEST['action'],array('new','edit')) and $saved!=true) {
	...
	if($relatedItemId)
	{
		if($_POST['rows'][$num]['common'])
		{
			...
			$_POST['rows'][$num]['common']['relatedTests'] = $commonColumns['related_tests'];
		}
		else
		{
			$commonColumns = cmfcMySql::load($sectionInfo['tableInfo']['tableName'], $sectionInfo['tableInfo']['columns']['relatedItem'], $relatedItemId);
	...
	if ($_POST['rows'][$num]['common']['relatedTests'])
	{
		$relatedTests = explode(';',$_POST['rows'][$num]['common']['relatedTests']);
		
		foreach ($relatedTests as $key1=>$row1)
		{
			 $tempQuery = 'SELECT `title`  AS title, `related_item` AS relatedItem FROM '.$_cp['sectionInfo']['testsTable']['tableName'].
				' WHERE '.$_cp['sectionInfo']['testsTable']['columns']['relatedItem']." = ".$row1.
				' AND '.$_cp['sectionInfo']['testsTable']['columns']['languageId']." = ".$translation->languageInfo['id'];
			 $temp = cmfcMySql::loadCustom($tempQuery);
			 
			$newRelatedTests[$key1]['title'] = $temp['title'];
			$newRelatedTests[$key1]['id'] = $temp['relatedItem'];
		}
		if ($newRelatedTests)
		{
			$relatedTestsHtml = '<ul id="relatedTestsList">';
			foreach ($newRelatedTests as $key1=>$row1)
			{
				$relatedTestsHtml .= '<li><img src="interface/images/min_on.gif" onclick="$(this).parent().remove();removeRelatedTest('.$row1['id'].');">&nbsp;'.$row1['title'].'</li>';
			}
			$relatedTestsHtml .= '</ul>';
		}
	}
	...
	{
		$relatedTestsHtml = '<ul id="relatedTestsList"></ul>';	
	}
	
	$infa->showCustomRow(wsfGetValue('relatedTests'), 
		'<input type="text" dir="rtl" value="" size="40" class="input" id="autocomplete_'.$num.'_relatedTests" name="rows['.$num.'][common][relatedTests]"/>'
		.$relatedTestsHtml
		.'<input type="hidden" id="rows_'.$num.'_common_relatedTests" name="rows['.$num.'][common][relatedTests]" value="'.$_POST['rows'][$num]['common']['relatedTests'].'" />',
    ...
    <script type="text/javascript">
		function removeRelatedTest(testId)
		{
			var currentProducts = ";"+$("#rows_<?php echo $num?>_common_relatedTests").val()+";";
			var reg = new RegExp("[;]{1}[" + testId + "]{1}[;]{1}");
			currentProducts = currentProducts.replace(reg,";");
			currentProducts = currentProducts.slice(1,currentProducts.length-1);
			$("#rows_<?php echo $num?>_common_relatedTests").val(currentProducts);
		}
		$(document).ready(function(){
			//var data = "Core Selectors Attributes Traversing Manipulation CSS Events Effects Ajax Utilities".split(" ");
			$("#autocomplete_<?php echo $num?>_relatedTests").autocomplete('testsList.php',{
				width: 300,
				multiple: true,
				matchContains: true,
			...
			$("#autocomplete_<?php echo $num?>_relatedTests").result(function(event, data, formatted) {
				if (data)
				{
					//$(this).parent().next().find("input").val(data[1]);
					var hidden = $("#rows_<?php echo $num?>_common_relatedTests");
					hidden.val( (hidden.val() ? hidden.val() + ";" : hidden.val()) + data[1]);
					$(this).val('');
					//hidden.val(data[1]);

					$("<li>").html(!data ? "No match!" : "<img src='interface/images/min_on.gif' onclick='$(this).parent().remove();removeRelatedTest("+data[1]+");'>&nbsp;"+formatted).appendTo("#relatedTestsList");
				}
			});
	...
	<form name="mySearchForm" action="?" method="get" style="margin:0px;padding:0px" enctype="text/plain">
		...
		<table id="option-1" class="option-table" width="100%" border="0" cellspacing="1" cellpadding="0">
			...
			<tr class="table-row1" align="<?php echo $langInfo['htmlAlign']?>" >
				...
				<td align="<?php echo $langInfo['htmlAlign']?>" ><input name="search[title]" class="input" type="text" value="<?php echo $_REQUEST['search']['title']?>" style="width:50%" /></td>
			</tr>
            <tr class="table-row2" >
				<td align="<?php echo $langInfo['htmlAlign']?>" ><?php echo wsfGetValue('formBody')?></td>
				<td  align="<?php echo $langInfo['htmlAlign']?>" ><input name="search[body]" class="input" type="text" value="<?php echo $_REQUEST['search']['body']?>" style="width:50%" /></td>
			</tr>
			<tr class="table-row2">
