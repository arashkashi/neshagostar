...
if ($_REQUEST['action']=='list') 
{
	...
	if (isset($_REQUEST['submit_search'])) {
		...
		if($_REQUEST['search']['question'] != "" && $_REQUEST['search']['answer'] != ""){
			
			$searchSqlWhereQuery = " AND (
				(`{td:question}` LIKE '%[question]%') AND
				(`{td:answer}` LIKE '%[answer]%' )
			)";
			
		}
		elseif($_REQUEST['search']['question'] != "")
		{
		
			$searchSqlWhereQuery = " AND (
				`{td:question}` LIKE '%[question]%')";
		...
		/* */
		$replacements=array(
			'{td:question}'=>$_cp['sectionInfo']['tableInfo']['columns']['question'],
			'{td:answer}'=>$_cp['sectionInfo']['tableInfo']['columns']['answer'],
			'[question]'=>$_REQUEST['search']['question'],
			'[answer]'=>$_REQUEST['search']['answer'],
...
if (in_array($_REQUEST['action'],array('new','edit')) and $saved!=true) {
	...
	wsfWysiwygLoader(array(
		...
		'editors'=>array(
			"rows[$num][columns][answer]"=>array('direction'=>$itemLanguage['direction']),			
		)
	));
	/* */
	...
	$infa->showTextAreaRow(wsfGetValue('question'),"rows[$num][columns][question]", $_POST['rows'][$num]['columns']['question'],'', '', 5, '90%');
	$infa->showTextAreaRow(wsfGetValue('answer'),"rows[$num][columns][answer]", $_POST['rows'][$num]['columns']['answer'],'', '', 15, '90%');
	/* */
	if ($_REQUEST['action'] == 'edit') {
	...
	<form name="mySearchForm" action="?" method="get" style="margin:0px;padding:0px" enctype="text/plain">
		...
		<table id="option-1" class="option-table" width="100%" border="0" cellspacing="1" cellpadding="0">
			...
			<tr class="table-row1" align="<?php echo $langInfo['htmlAlign']?>" >
				<td width="200"><?php echo wsfGetValue('question')?> </td>
				<td align="<?php echo $langInfo['htmlAlign']?>" ><input name="search[question]" class="input" type="text" value="<?php echo $_REQUEST['search']['question']?>" style="width:50%" /></td>
			</tr>
            <tr class="table-row2" >
				<td align="<?php echo $langInfo['htmlAlign']?>" ><?php echo wsfGetValue('answer')?></td>
				<td  align="<?php echo $langInfo['htmlAlign']?>" ><input name="search[answer]" class="input" type="text" value="<?php echo $_REQUEST['search']['answer']?>" style="width:50%" /></td>
			</tr>
			<tr class="table-row2">
		...
		<table id="listFormTable" dir=<?php echo $langInfo['htmlDir']?>  class="table" width="100%" border="1" cellspacing="0" cellpadding="0" style="border-color: #d4dce7" >
			...
			<tr>
				...
				<td align="<?php echo $langInfo['htmlAlign']?>" nowrap="nowrap" class="table-title field-title">
					<a href="<?php echo $paging->getSortingUrl('question','DESC')?>">
						<span style="font-family:arial">▼</span>
					</a>
					<a href="<?php echo $paging->getSortingUrl('question','ASC')?>">
						<span style="font-family:arial">▲</span>
					</a>
					<?php echo wsfGetValue(question)?>
				</td>
                <?php /*
				if ($_cp['sectionInfo']['tableInfo']['columns']['categoryId'])
				{
				...
				?>
				<td align="<?php echo $langInfo['htmlAlign']?>" nowrap="nowrap" class="table-title field-title" >
					<a href="<?php echo $paging->getSortingUrl('answer','DESC')?>">
						<span style="font-family:arial">▼</span>
					</a>
					<a href="<?php echo $paging->getSortingUrl('answer','ASC')?>">
						<span style="font-family:arial">▲</span>
					</a>
					<?php echo wsfGetValue(answer)?>
				</td>
			</tr>
			<?php 			foreach ($rows as $key=>$row) 
			{
				...
				<td class="field-title" align="<?php echo $langInfo['htmlAlign']?>" > 
					<?php echo cmfcString::briefText(strip_tags($row['question']), 100);?>
				</td>
                <?php
				/*
                if ($_cp['sectionInfo']['tableInfo']['columns']['categoryId'])
				...
				?>
				<td class="field-title" align="<?php echo $langInfo['htmlAlign']?>" > 
					<?php echo cmfcString::briefText(strip_tags($row['answer']), 100);?>
				</td>
