...
//<!-- Save Order...
	...
	if ($_REQUEST['action'] =='save_order' ) {
		...
		foreach ($_POST['rows'] as $id => $row) {
			$column = array($sectionInfo['tableInfo']['columns']['orderNumber'] => $row['orderNumber']);
			cmfcMySql::update(
		...
		$messages['messages'][] = wsfGetValue("orderSaved");
...
if ($_REQUEST['action']=='list') {
	...
	if (isset($_REQUEST['submit_search'])) {
		...
		if($_REQUEST['search']['title'] != "" && $_REQUEST['search']['ip'] != ""){
		...
		if ($_REQUEST['search']['userId'])
		{
			$searchSqlWhereQuery .= " AND (`{td:userId}` = '[userId]')";
		}
		/* */
		$replacements=array(
			'{td:title}'=>$_cp['sectionInfo']['tableInfo']['columns']['title'],
			'{td:ip}'=>$_cp['sectionInfo']['tableInfo']['columns']['ip'],
			'{td:userId}'=>$_cp['sectionInfo']['tableInfo']['columns']['userId'],
			'[title]'=>$_REQUEST['search']['title'],
			'[ip]'=>$_REQUEST['search']['ip'],
...
if (in_array($_REQUEST['action'],array('new','edit')) and $saved!=true) {
	...
	$infa->showTableHeader( wsfGetValue('commonInfo') );
	...
	$infa->showInputRow(wsfGetValue('url'), "rows[$num][common][url]", $_POST['rows'][$num]['common']['url'], '',40, 'ltr'); 
	if ($_cp['sectionInfo']['tableInfo']['columns']['photoFilename']){
		$infa->showCustomRow(
			wsfGetValue('photo'),
...
?>
	<form name="mySearchForm" action="?" method="get" style="margin:0px;padding:0px" enctype="text/plain">
		...
		<table id="option-1" class="option-table" width="100%" border="0" cellspacing="1" cellpadding="0">
			...
			<tr class="table-row1" align="<?php echo $langInfo['htmlAlign']?>" >
				<td width="200"><?php echo wsfGetValue('ip')?> </td>
				<td align="<?php echo $langInfo['htmlAlign']?>" ><input name="search[ip]" class="input" type="text" value="<?php echo $_REQUEST['search']['ip']?>" style="width:50%; direction: ltr;" /></td>
			</tr>

            <tr class="table-row2" >
				<td align="<?php echo $langInfo['htmlAlign']?>" ><?php echo wsfGetValue('user')?></td>
				<td  align="<?php echo $langInfo['htmlAlign']?>" >
                <?php
				$items = cmfcMySql::getRowsWithMultiKeys(
					$_cp['sectionInfo']['usersTable']['tableName'],
					array(
						$_cp['sectionInfo']['usersTable']['columns']['languageId'] => $translation->languageInfo['id'],
					)
				);
				
				$items = cmfcMySql::getRows($_cp['sectionInfo']['usersTable']['tableName']);
				//cmfcHtml::printr($items);
				echo cmfcHtml::drawDropDown(
					"search[userId]",
					$_REQUEST['search']['userId'],
					$items,
					'id',
					'full_name',
					NULL,
		...
		<table id="listFormTable" dir=<?php echo $langInfo['htmlDir']?>  class="table" width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#d4dce7">
			...
			<tr>
                ...
                <td align="<?php echo $translation->languageInfo['align']?>" nowrap="nowrap" class="table-title field-title" >
					<?php echo wsfGetValue('user')?>
				</td>
				<td align="<?php echo $translation->languageInfo['align']?>" nowrap="nowrap" class="table-title field-title" >
					<a href="<?php echo $paging->getSortingUrl('occurrence_date_time','DESC')?>">
						<span style="font-family:arial">▼</span>
					</a>
					<a href="<?php echo $paging->getSortingUrl('occurrence_date_time','ASC')?>">
						<span style="font-family:arial">▲</span>
					</a>
					<?php echo wsfGetValue('occurrenceDateTime')?>
				</td>
				<td align="<?php echo $translation->languageInfo['align']?>" nowrap="nowrap" class="table-title field-title" >
					<a href="<?php echo $paging->getSortingUrl('ip','DESC')?>">
						<span style="font-family:arial">▼</span>
					</a>
					<a href="<?php echo $paging->getSortingUrl('ip','ASC')?>">
						<span style="font-family:arial">▲</span>
					</a>
					<?php echo wsfGetValue('ip')?>
				</td>
				<?php /*
				<td align="<?php echo $langInfo['htmlAlign']?>" nowrap="nowrap" class="table-title field-title" >
			...
			<?php 			foreach ($rows as $key=>$row) {
                ...
                <?php
					$userId = cmfcMySql::load(
						$_ws['physicalTables']['users']['tableName'], 
						$_ws['physicalTables']['users']['columns']['id'],
						$row['userId']
						);
					$userId = $userId['full_name'];
					//cmfcHtml::printr($userId);
				?>
                <td class="field-title" align="<?php echo $langInfo['htmlAlign']?>" > 
					<?php echo $userId;?>
				</td>
                <td class="field-title" align="<?php echo $translation->languageInfo['align']?>" > 
					<?php echo wsfGetDateTime('d M Y (H:i)', $row['occurrenceDateTime'], $langInfo['sName']);?>
				</td>
                <td class="field-title" align="<?php echo $translation->languageInfo['align']?>" > 
					<?php echo $row['ip'];?>
				</td>
