...
if (!$_POST['itemLanguage'])
	$ll = $translation->languageInfo['id'];
else
	$ll = $_POST['itemLanguage'];
...
{
	if ( (string)(int)($_REQUEST['categoryId']) == (string)($_REQUEST['categoryId']) )
	{
		$related = cmfcMySql::loadWithMultiKeys(
			$_cp['sectionInfo']['tableInfo']['tableName'],
			array(
					$_cp['sectionInfo']['tableInfo']['columns']['categoryId']=>	$_REQUEST['categoryId'],
					$_cp['sectionInfo']['tableInfo']['columns']['languageId']=> $ll	,
			)
		);
	}
	else
	{
		$related = cmfcMySql::loadWithMultiKeys(
			$_cp['sectionInfo']['tableInfo']['tableName'],
			array(
					$_cp['sectionInfo']['tableInfo']['columns']['referringId']=>	$_REQUEST['categoryId'],
					$_cp['sectionInfo']['tableInfo']['columns']['languageId']=> 	$ll	,
			)
		);
	}
...
if ( (string)(int)($_REQUEST['categoryId']) == (string)($_REQUEST['categoryId']) )
{
	$editable = cmfcMySql::getRowsWithMultiKeys(
		$_cp['sectionInfo']['tableInfo']['tableName'],
		array(
			$_cp['sectionInfo']['tableInfo']['columns']["categoryId"] => $_REQUEST['categoryId'],
...
if ((isset($_POST['submit_save']) and $_GET['action']!='view') || isset($_POST['saveEditNextLang'])) 
{
	...
	{		
		...
		if ($row['action']!='update')
		{
			$_columns['id'] = $relatedItemId;
		}
		else
		{
			unset($_columns['id']);
			$_GET['id'] = $relatedItemId;
		}
		...
		if (empty($validateResult)) 
		{
			...
			$newColumnsValues = cmfcMySql::convertColumnNames($_commonColumns,$_cp['sectionInfo']['tableInfo']['columns']);
			
			cmfcMySql::update(
				$_cp['sectionInfo']['tableInfo']['tableName'],
				$_cp['sectionInfo']['tableInfo']['columns']['categoryId'],
				$newColumnsValues,
				$_REQUEST['categoryId']
			);
		...
		if($itemLanguage['id'] )
		{
			if ( (string)(int)($_REQUEST['categoryId']) == (string)($_REQUEST['categoryId']) )
			{
				...
				$sqlQuery = "SELECT * FROM ".$_cp['sectionInfo']['tableInfo']['tableName'].
							" WHERE language_id = '".$itemLanguage['id'].
							"' AND category_id = '".$_REQUEST['categoryId']."'";
			}
			else
			{
				$sqlQuery = "SELECT * FROM ".$_cp['sectionInfo']['tableInfo']['tableName'].
							" WHERE language_id = '".$itemLanguage['id'].
							"' AND referring_id = '".$_REQUEST['categoryId']."'";	
			}
		}
		else
		{
...
if (in_array($_REQUEST['action'],array('new','edit')) and $saved!=true) 
{
	if($relatedItemId)
	{
		...
		{
			$commonColumns = cmfcMySql::load($_cp['sectionInfo']['tableInfo']['tableName'], $sectionInfo['tableInfo']['columns']['relatedItem'], $relatedItemId);
			$_POST['rows'][$num]['common']['photoFilename'] = $commonColumns['photo_filename'];
		}
		else
		{
			$commonColumns = cmfcMySql::load($_cp['sectionInfo']['tableInfo']['tableName'], $_cp['sectionInfo']['tableInfo']['columns']['id'], $relatedItemId);
	...
	$infa->showHiddenInput("rows[$num][columns][categoryId]", $_REQUEST["categoryId"]);
	

	if ($_REQUEST['action'] == 'edit') 
	{
		$infa->showHiddenInput("rows[$num][action]", "update");
		//$infa->showHiddenInput("rows[$num][columns][id]", $_REQUEST['id']);
	}
	elseif($_REQUEST['action'] == 'new') 
	{
		$infa->showHiddenInput("rows[$num][columns][referringId]", $_REQUEST["categoryId"]);
