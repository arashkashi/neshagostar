...
if ((isset($_POST['submit_save']) and $_GET['action']!='view') || isset($_POST['saveEditNextLang'])) {
	...
	foreach ($_POST['rows'] as $num=>$row) {		
		...
		if ($_cp['sectionInfo']['tableInfo']['columns']['filename']){
			$_commonColumns['filename'] = wsfUploadFileAuto(
				"rows[$num][common][filename]",
				$_ws['siteInfo']['path'].$_cp['sectionInfo']['folderRelative']
			);
		}
		
		if(!$_commonColumns['active'])
			$_commonColumns['active'] = 0;
...
if (in_array($_REQUEST['action'],array('new','edit')) and $saved!=true) {
	...
	if($relatedItemId)
	{
		if($_POST['rows'][$num]['common'])
		{
			...
			$_POST['rows'][$num]['common']['filename'] = $commonColumns['filename'];
		}
		else
		{
			$commonColumns = cmfcMySql::load($sectionInfo['tableInfo']['tableName'], $sectionInfo['tableInfo']['columns']['relatedItem'], $relatedItemId);
	...
	$infa->showTextAreaRow(wsfGetValue('questionTitle'),"rows[$num][common][title]", $_POST['rows'][$num]['common']['title'],'', '', 15, '90%');
	
	if ($_cp['sectionInfo']['tableInfo']['columns']['active'])
	{
		$infa->showCheckBoxRow(wsfGetValue('active'), "rows[$num][common][active]" , $_POST['rows'][$num]['common']['active']);
	}
	...
	?>
		<table id="listFormTable" dir=<?php echo $langInfo['htmlDir']?>  class="table" width="100%" border="1" cellspacing="0" cellpadding="0" style="border-color: #d4dce7" >
			...
			<?php 			foreach ($rows as $key=>$row) {
                ...
                <td class="field-title" align="<?php echo $langInfo['htmlAlign']?>" id="activeFlag<?php echo $row['id']?>"> 
                    <?php 
                    if ($row['active'])
                    {?>
                        <a href="<?php echo $_SERVER['REQUEST_URI']?>&fn=active&targetItemId=<?php echo $row['relatedItem']?>&currentValue=<?php echo $row['active']?>"
                        <?php 
                        if ($ajax)
                        {?>
                            onclick="<?php echo $ajax->jsfSimpleCallForATagOnClick(array('activeFlag'.$row['id'],'pageMessages'))?>"
                            <?php }?>
                        >
                            <img src="interface/images/tick.png" id="active<?php echo $row['id']?>" alt="<?php echo $translation->getValue('active')?>" style="cursor:pointer; border:0px;" />
                        </a>
                     <?php
                     }
                     else
                     {?>
                        ...
                        <?php }?>
                        >
                            <img src="interface/images/bullet_red.png" id="active<?php echo $row['id']?>" alt="<?php echo $translation->getValue('active')?>" style="cursor:pointer; border:0px;" />
                        </a>
                     <?php }
