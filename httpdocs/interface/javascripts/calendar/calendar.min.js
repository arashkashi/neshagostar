...
Date.prototype.getUTCWeekNumber = function() {
	...
	var DoW = d.getUTCDay();
	d.setUTCDate(d.getUTCDate() - (DoW + 6) % 7 + 3);
	var ms = d.valueOf();
	d.setUTCMonth(0);
	d.setUTCDate(4);
...
Date.prototype.print = function (str, dateType, useLangNumbers) {
	...
	s["%a"] = Calendar._SDN[w]; 
	s["%A"] = Calendar._DN[w];
	s["%b"] = (dateType == 'jalali' ? Calendar._JSMN[m] : Calendar._SMN[m]);
	s["%B"] = (dateType == 'jalali' ? Calendar._JMN[m] : Calendar._MN[m]); 
	...
	s["%u"] = this.getLocalDay(true, dateType) + 1;	
	s["%w"] = this.getLocalDay(true, dateType);	
	s["%y"] = ('' + y).substr(2, 2);
	s["%Y"] = y;
	s["%%"] = "%";
	var re = /%./g;
