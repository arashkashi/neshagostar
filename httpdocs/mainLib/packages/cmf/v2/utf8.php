...
/**
  ...
  * @package cmf
  * @subpackage beta
  * @todo should have cmfc prefix as of generallib3
  * @revision $WCREV$@$WCDATE$
  * @revisionRange $WCRANGE$
...
class cmfcUtf8 {	
	...
	/**  HAS BUG BUG BUG
	 * split an UTF-8 string
	 * just like PHP str_split
	 */
	function str_split($string,$length) {
		$totalLength=UTF8::strlen($string);
		$slices=array();
		for ($start=0;$start<=$totalLength;$start++) {
			$slices[]=UTF8::substr($str, $start, $length);
		}
		return $slices;
	}
	
	function explodeWords($string) {
		return preg_split("/[\s,]+/", $string);
	}
	
	function justifySimple($string,$charsPerLine,$expletive=' ') {
		$words=UTF8::explodeWords($string);
		$result='';
		$line='';
		$lines=array();
		$length=0;
		...
		foreach ($words as $word) {
			$wordLength=UTF8::strlen($word);
			if ($length>0) $wordLength+=1;
			if ($length+$wordLength<=$charsPerLine) {
				$line.=($length==0)?'':' ';
				$line.=$word;
				$length+=$wordLength;
			} else {
				$lines[]=$line.str_repeat($expletive,$charsPerLine-$length+1);
				$line=$word;
				$length=$wordLength;
			}
		}
