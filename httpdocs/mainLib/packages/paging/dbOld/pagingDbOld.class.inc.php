...
class cmfcRichPagingOld
{
	...
	var $qsvn_sort_ascending='sort_type';
	...
	function cmfcRichPagingOld($total=null,$limit=40,$sqlQuery=null)
	{
		if (!is_null($sqlQuery)) {$total=cmfcMySql::getTotalRowsNumberViaSqlQuery($sqlQuery);}
		$this->total=$total;
		$this->limit=$limit;
	...
	{
		$this->set_variables();
		if (is_null($sqlQuery)) {$sqlQuery=$this->sql_query;}
	...
	function get_sorted_query($sqlQuery=null)
	{
		if ($this->auto_recognize) {
			if (!empty($_GET[$this->qsvn_sort_ascending]))
				$this->sort_ascending=$_GET[$this->qsvn_sort_ascending];
			else
		...
		if (!empty($this->sort_by))
			return cmfcMySql::getSortedQuery($sqlQuery,$this->sort_by,$this->sort_ascending);
		else
			return $sqlQuery;
	}

	function get_next_limited_query() {
		$sqlQuery=$this->sql_query;
	...
	//$sort_ascending you can pass 'ASC' & 'DESC' or false & true
	function get_sorting_query_string($sort_by=null,$sort_ascending=null)
	{
		if (strtoupper($sort_ascending)=='ASC'){$sort_ascending=true;}
		if (strtoupper($sort_ascending)=='DESC'){$sort_ascending=false;}
		if (is_null($sort_by)) {$sort_by=$this->sort_by;}
		if (is_null($sort_ascending)) {$sort_ascending=$this->sort_ascending;}
		if ($this->sorting_enabled==true)
			$result="&$this->qsvn_sort_by=$sort_by&$this->qsvn_sort_ascending=$sort_ascending";
	...
	function get_query_string()
	{
		...
		if (!is_null($sort_ascending)) {$this->sort_ascending=$sort_ascending;}
		if ($sort_ascending='ASC'){$sort_ascending=false;}
		if ($sort_ascending='DESC'){$sort_ascending=true;}
		if ($this->sorting_enabled==true)
			$result="&$this->qsvn_sort_by=$this->sort_by&$this->qsvn_sort_ascending=$this->sort_ascending";
		$result.="&$this->qsvn_from=$this->from&$this->qsvn_to=$this->to";
		return $result;
	}
...
//			$total=$this->total+$this->begin_value;
	...
	function get_page_number_by_from($from=null) {
		...
		$page_number=floor(($from+$this->limit+1)/$this->limit);
